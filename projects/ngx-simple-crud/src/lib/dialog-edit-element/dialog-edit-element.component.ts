...
export class DialogEditElementComponent implements OnInit {
  ...
  getFirstControlValue(control: EditDialogDataInterface['controls'][0]) {
    ...
    if (typeof control.inputMask === 'function' && this.data.element[control.key]) {
  ...
  inputMask(controlName: string, event: KeyboardEvent) {
    ...
    if (control && typeof control.inputMask === 'function') {
      this.form.controls[controlName].setValue(control.inputMask((event.target as any).value, event));
    }
  }
